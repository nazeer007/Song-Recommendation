...
main = tkinter.Tk()
main.title("Transformers Based Speech Emotion Detection") #designing main screen
...
def recommendSongs(emotion):
    global tf1, songs
    print(f"Recommendation for emotion: {emotion}")
    if emotion == 'calm' or emotion == 'happy' or emotion == 'neutral':
        ...
        print("Songs for happy/calm/neutral:", songs)
        tf1['values'] = songs
        tf1.current(0)
    else:
        names = ""
        ...
        names = names.split(",")
        songs = list(set([record for record in names]))
        print("Songs for other emotions:", songs)
        tf1['values'] = songs
        tf1.current(0)

def demandRecommendation():
    global main, tf1, songs
    textarea.delete('1.0', END)
    ...
    # Clear the existing values in the dropdown
    tf1.set('')

    # Update the values in the dropdown
    tf1['values'] = tuple(songs)
...
def predictEmotion():
    ...
    recording = sd.rec(int(3 * sample_rate), samplerate=sample_rate, channels=1, dtype=float, blocking=True)
    ...
    print("Predicted emotion index:", predictions)
...
def runGUI():
    ...
    font = ('times', 16, 'bold')
    title = Label(main, text='Chatbot Song Recommendation System', justify=LEFT)
